...
def battle(boss, pl):
    ...
    player_damage = max(1, pl[1] - boss[2])
    boss_damage = max(1, boss[1] - pl[2])
...
while current_weapon < len(weapons):
    ...
    # Check if the player wins the battle, and compare the gold spent to the minimum
    if battle(boss_base_stats, player) and (gold < minimum_gold or minimum_gold == -1):
        minimum_gold = gold
...
print('The player only needed to spend', minimum_gold, 'gold to win!')
