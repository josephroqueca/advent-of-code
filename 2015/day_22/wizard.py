...
class Battle:
	...
	def can_cast(self, spell):
		if self.player.mana < spells[spell]['cost']: return False
		if spell == 'shield' and self.shield_turns > 1: return False
		if spell == 'recharge' and self.regen_turns > 1: return False
		if spell == 'poison' and self.poison_turns > 1: return False
		return True
...
while battles:
	...
	# No need to continiue if the mana cost won't be the minimum
	...
	for index, next_battle in enumerate(next_battles):
		...
		if next_battle.check_game_over():
			if next_battle.boss.hp <= 0:
				minimum_mana = min(minimum_mana, next_battle.total_cost) if minimum_mana != -1 else next_battle.total_cost
			continue
